...
function DebateCard({ debate }: { debate: Debate }) {
  const statusColors = {
    pending: 'text-slate-500 bg-slate-100 dark:text-slate-400 dark:bg-slate-800',
    analyzing: 'text-amber-600 bg-amber-100 dark:text-amber-400 dark:bg-amber-900/30 animate-pulse',
    complete: 'text-emerald-600 bg-emerald-100 dark:text-emerald-400 dark:bg-emerald-900/30',
    error: 'text-rose-600 bg-rose-100 dark:text-rose-400 dark:bg-rose-900/30',
  };

  return (
    <Link href={`/debate/${debate.id}`}>
      <div className="bg-white border border-slate-200 rounded-lg p-4 hover:border-slate-400 hover:bg-slate-50 dark:bg-slate-800/50 dark:border-slate-700/50 dark:hover:border-slate-500 dark:hover:bg-slate-800 transition-all cursor-pointer group">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-slate-900 truncate group-hover:text-slate-700 dark:text-slate-100 dark:group-hover:text-white">
              {debate.title}
            </h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              <span className="text-blue-600 dark:text-blue-400">{debate.debater_a}</span>
              <span className="mx-2 text-slate-400 dark:text-slate-600">vs</span>
      ...
      <header className="border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-slate-900 dark:text-white text-lg leading-none">DebateRank</h1>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 py-8">
        ...
        {!showUpload && debates.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-600 max-w-lg mx-auto mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-2xl mx-auto mb-8 text-left">
              {[
                ...
                <div key={f.title} className="bg-white border border-slate-200 dark:bg-slate-800/50 dark:border-slate-700/50 rounded-lg p-3">
        ...
        {showUpload && (
          <div className="max-w-2xl mx-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">New Debate Session</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                  Paste or upload a debate transcript to begin analysis
                </p>
              </div>
            </div>
            <div className="bg-white border border-slate-200 dark:bg-slate-900/50 dark:border-slate-700/50 rounded-xl p-6">
              <DebateUpload />
            </div>
          </div>
        )}
