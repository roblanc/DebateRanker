...
export default function HomePage() {
  ...
  const [status, setStatus] = useState<'idle' | 'fetching' | 'creating' | 'error'>('idle');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. fetch transcript + title from YouTube
      ...
      if (!ytRes.ok) throw new Error(ytData.error || 'Could not fetch transcript');

      setStatus('creating');

      // 2. create debate with auto defaults
      ...
      // 3. trigger analysis (fire and forget)
      fetch('/api/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
